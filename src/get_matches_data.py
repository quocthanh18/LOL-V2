...
def main():
    ...
    matches = list(set(list(open("data/matches_history.txt", "r").read().splitlines())))
    matches = matches[matches.index("KR_7022796313") + 1:]
    with open("data/matches_data.csv", "a") as f:
        ...
        while len(matches) > 0:
            try:
                #Get match response
                matchId = matches[0]
                filtered = f"https://asia.api.riotgames.com/lol/match/v5/matches/{matchId}?api_key={riot_api_key}"
                mode = requests.get(filtered).json()["info"]["gameMode"]
